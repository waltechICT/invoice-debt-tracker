...
<div class="container-fluid py-2">
    <div class="mb-4">
        <h2 class="fs-4 fw-bold text-dark mb-1">{{ 'Edit Tenant' }}</h2>
        <p class="text-muted small mb-0">Update the tenant workspace details below.</p>
    </div>
    ...
    <div class="card border border-light shadow-sm rounded-4 overflow-hidden">
        <div class="card-header bg-white p-4 border-bottom d-flex flex-wrap gap-2 justify-content-between align-items-center">
            <h5 class="fw-bold text-dark mb-0 fs-6">Tenant Details</h5>
            ...
            <form action="{{ route('tenant.update', $tenant->id) }}" method="post">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="slug" id="slug"
                        ...
                        value="{{ old('slug', $tenant->slug) }}" placeholder="Enter tenant slug" required>
                    @error('slug')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="subdomain" id="subdomain"
                        ...
                        value="{{ old('subdomain', $tenant->subdomain) }}" placeholder="Enter tenant subdomain">
                    @error('subdomain')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="metadata" id="metadata" rows="4"
                        ...
                        placeholder="{&quot;color&quot;:&quot;blue&quot;,&quot;timezone&quot;:&quot;UTC&quot;}">{{ old('metadata', json_encode($tenant->metadata ?? [], JSON_PRETTY_PRINT)) }}</textarea>
                    @error('metadata')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="is_active" id="is_active" class="form-select @error('is_active') is-invalid @enderror" required>
                        <option value="1" {{ old('is_active', $tenant->is_active) ? 'selected' : '' }}>Yes</option>
                        <option value="0" {{ ! old('is_active', $tenant->is_active) ? 'selected' : '' }}>No</option>
                    </select>
                    @error('is_active')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                ...
                <div class="d-grid d-md-block mt-3">
                    <button type="submit" class="btn btn-primary">
                        <i class="bi bi-save me-2"></i> Update Tenant
                    </button>
                </div>
            </form>
        </div>
